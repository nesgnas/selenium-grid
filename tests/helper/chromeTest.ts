...
export async function runChromeTest(scenario: (driver: any) => Promise<void>, scenarioName: string) {
    // Use the HUB_URL from the environment variables
    const hubUrl = process.env.HUB_URL || "http://localhost:4444/wd/hub";

    const driver = await new Builder().forBrowser("chrome").usingServer(hubUrl).build();
